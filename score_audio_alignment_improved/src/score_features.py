...
def score_to_basic_chroma(
    note_array: np.ndarray, frame_times: np.ndarray
) -> np.ndarray:
    onsets = note_array["onset_beat"].astype(float)
    durations = note_array["duration_beat"].astype(float)
    offsets = onsets + durations
...
def compute_score_features(
    ...
    backend: str = "basic_chroma",
) -> np.ndarray:
    if backend == "basic_chroma":
        return score_to_basic_chroma(note_array, frame_times)

    raise ValueError(f"Unknown score backend: {backend}")
